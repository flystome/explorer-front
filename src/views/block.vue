<template>
  <section id='block' class="w1200">
    <h3>{{$t('global.block')}} <span>#{{block.number}}</span></h3>
    <div class="box">
      <p class="title">{{$t('global.overview')}}</p>
      <div class="infos" v-if="!block.number">
        <div class="loading">
      ...
      <div class="infos" v-else>
        <div class="detailBox">
          ...
          <div class="detail">
            ...
            <router-link :to="`/block/${+block.number + 1}`" class="changeblock">
        ...
        <div class="detailBox">
          ...
          <p class="detail">
            <i></i>
            <span> {{ block.timestamp | formatAgo }} {{$t('global.ago')}} ({{new Date(block.timestamp*1000).toLocaleString()}})</span>
          </p>
        </div>
        <div class="detailBox">
          <p class="name">{{$t('global.transactions')}}:</p>
          <p class="detail">
            <router-link :to="`/txs/${block.hash}`">{{ block.txnCount }}{{$t('desc.transactions')}}</router-link>{{$t('global.and')}} 
            <router-link to="">{{ block.ctxnCount }}{{$t('desc.contractInternal')}}</router-link>
            <span>{{$t('desc.inBlock')}}</span>
          </p>
        </div>
        <div class="detailBox">
          <p class="name">{{$t('block.minedBy')}}:</p>
          <p class="detail">
            <router-link :to="`/address/${block.miner}`">{{ block.miner }}</router-link>
